#!/usr/bin/env bash
#
# mayhem/build.sh — build the python-minifier Atheris fuzz harness + its standalone
...
#      re-run resolves entirely from it (pip --no-index --find-links). python_minifier itself
#      stays the editable source tree ($SRC/src, exposed via PYTHONPATH) so a PATCH agent's
#      edits take effect with no reinstall.
#   2. Compile launcher.c -> the ELF Mayhem target `fuzz_min` (Atheris is a Python script;
#      Mayhem needs an ELF cmd, and the gate needs DWARF < 4 — hence a compiled wrapper).
#   3. Build the same launcher as the standalone (run-once) reproducer `fuzz_min-standalone`.
...
# python_minifier is an src-layout package ($SRC/src/python_minifier); expose it (and the site dir)
# on PYTHONPATH so a PATCH agent's edits under src/ take effect immediately with no reinstall.
PYRUN="$SITE:$SRC/src"
...
PYTHONPATH="$PYRUN" "$PY" -c 'import atheris, pytest, python_minifier; print("imports OK")'
...
#    run time (the Dockerfile sets ENV PYTHONPATH), so the Python side finds atheris + python_minifier.
HARNESS="$SRC/mayhem/fuzz_min.py"
echo ">> compiling fuzz_min (+ standalone) with DEBUG_FLAGS=$DEBUG_FLAGS"
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/fuzz_min"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" -DHARNESS="\"$HARNESS\"" \
    "$SRC/mayhem/launcher.c" -o "$SRC/fuzz_min-standalone"
...
$CC $DEBUG_FLAGS -DPYTHON="\"$PY\"" "$SRC/mayhem/run_tests.c" -o "$SRC/python_minifier_run_tests"

echo ">> build.sh complete"
ls -la "$SRC/fuzz_min" "$SRC/fuzz_min-standalone" "$SRC/python_minifier_run_tests"
